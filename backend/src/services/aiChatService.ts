...
export class AIChatService {
   ...
   * Implements Monad (rule-based) -> GROQ AI -> Fallback pattern
   */
  async getChatResponse(message: string, conversationHistory: ChatMessage[] = []): Promise<string> {
    ...
    if (this.groqClient) {
      try {
        ...
        console.error("GROQ AI error, falling back to Monad AI:", error);
        // Fall through to Monad AI fallback
      }
    }

    // Fallback to Monad AI (rule-based pattern matching)
...
- Explain the rewards system (100 points = 1 MON)
...
Keep responses concise, friendly, and focused on gaming help. Use emojis sparingly but appropriately.`;
      ...
      throw error; // Re-throw to let caller handle fallback to Monad AI
    }
  }

  /**
   * Fallback response when AI is not available (Monad-style pattern matching)
   */
  private getFallbackResponse(message: string): string {
    ...
    if (lowerMessage.includes("reward") || lowerMessage.includes("mon") || lowerMessage.includes("earn")) {
